/* =========================================================================
   영어 끝말잇기 — 통신 계층 (net.js)

   ⚠ 이 파일은 `games/catchmind/net.js`를 **복사해 솔트만 바꾼 것**이다.
      바꾼 것: SALT_TOPIC / SALT_KEY (방 코드가 같아도 캐치마인드 방과 절대 겹치지
      않게 한다), 전역 이름 CM_NET → WB_NET, clientId 접두사.
      로직은 손대지 않았다 — 검증된 코드를 그대로 쓰는 편이 안전하다.
      단점: 캐치마인드 쪽에서 통신 버그를 고쳐도 여기로 자동 전파되지 않는다.
      통신 문제가 생기면 캐치마인드 net.js와 먼저 대조할 것.

   ⚠ crypto.subtle은 보안 컨텍스트에서만 동작한다 → **멀티는 https에서만 된다.**
      `file://` 더블클릭에서는 멀티가 막히지만 **싱글(AI 대전)은 그대로 동작한다.**
      싱글이 네트워크에 전혀 의존하지 않게 짠 이유다.
...
var SALT_TOPIC='wbwc-v1-topic-salt';
var SALT_KEY='wbwc-v1-key-salt';
  ...
  function attempt(){
    ...
    try{
      c=window.mqtt.connect(url,{
        clientId:'wb_'+Math.random().toString(16).slice(2,10),
...
window.WB_NET={ make:makeMqttNet, brokers:BROKERS, cryptoOk:cryptoOk, topicId:topicId };
/* game.js가 만든 인스턴스의 진단 함수만 노출한다 (콘솔에서 window.__WBNET.info()).
   인스턴스를 통째로 내보내면 pub()까지 열려서, 같은 방 참가자가 방장 상태 메시지를
   한 줄로 위조할 수 있다. 키는 이미 링크에 있으니 원리상 막을 수는 없지만
   그 문턱을 콘솔 한 줄로 낮춰 줄 이유는 없다. */
var _mk=window.WB_NET.make;
window.WB_NET.make=function(){ var n=_mk(); window.__WBNET={info:n.info}; return n };
})();
